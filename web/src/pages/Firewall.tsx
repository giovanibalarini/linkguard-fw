...
export default function Firewall() {
  ...
  const run = async (fn: () => Promise<any>, ok: string) => {
    ...
    try {
      ...
      notify(t('fwx.error', { msg: e.response?.data?.error || e.message }), 'error');
    } finally {
      setBusy(false);
    }
  };

  const handleBackup =() => run(() => client.post('/api/nftables/backup', { label: '' }), t('fwx.toast.snapshot.created'));
  ...
  const handleRollback = (b: IptablesBackup) => confirm(t('fwx.backups.rollback.confirm', { label: b.label })) && run(() => client.post('/api/nftables/rollback', { backup_id: b.id }), t('fwx.toast.rollback.done'));
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex gap-2">
          {canWrite && (
            <button onClick={handleBackup} disabled={busy} className="btn-secondary flex items-center gap-2 disabled:opacity-50">
              <Database className="w-4 h-4" /> {t('fwx.btn.snapshot')}
            </button>
          )}
      ...
      {loading ? (
        ...
        <Panel className="p-0 overflow-hidden">
          ...
          {ruleset.trim() ? <pre className="p-4 overflow-x-auto text-xs font-mono text-gray-300 leading-relaxed whitespace-pre">{ruleset}</pre> : <p className="p-8 text-center text-gray-600 text-sm">{t('fwx.ruleset.empty')}</p>}
        </Panel>
      ) : (
        <>
          ...
          <p className="text-gray-500 text-xs">
            {t('fwx.backups.explain.1')}{' '}
            {t('fwx.backups.explain.2')}
          </p>
          <Panel>
          {backups.length === 0 ? (
            <div className="text-center py-12"><Download className="w-12 h-12 text-gray-700 mx-auto mb-3" /><p className="text-gray-400">{t('fwx.backups.empty')}</p></div>
          ) : (
            <>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {canWrite && (
                        <IconButton
                          ...
                          disabled={busy}
                          label={t('fwx.backups.restore')}
                          className="shrink-0"
                        />
                      )}
                    </div>
                    <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
                      <dt className="text-gray-500">{t('fwx.backups.col.size')}</dt>
                      <dd className="text-gray-400 font-mono">{(b.rules.length / 1024).toFixed(1)} KB</dd>
                      <dt className="text-gray-500">{t('fwx.backups.col.date')}</dt>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-left text-gray-500 border-b border-gray-800"><th className="pb-3 pr-4 font-medium">{t('fwx.backups.col.label')}</th><th className="pb-3 pr-4 font-medium">{t('fwx.backups.col.size')}</th><th className="pb-3 pr-4 font-medium">{t('fwx.backups.col.date')}</th>{canWrite && <th className="pb-3 font-medium">{t('fwx.backups.col.actions')}</th>}</tr></thead>
                  <tbody>
                    {backups.map((b) => (
