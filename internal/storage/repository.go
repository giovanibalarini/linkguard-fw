...
// DashboardGridColumns é a largura da grade do painel. O mesmo número vale no
// frontend: um item só existe entre a coluna 0 e esta.
const DashboardGridColumns = 12

// DashboardMaxItems limita quantos itens um layout pode ter. É teto de sanidade,
...
const DashboardMaxItems = 64

// DashboardMaxRowSpan limita a altura de um item, pelo mesmo motivo.
const DashboardMaxRowSpan = 24
...
// DashboardWidget é uma entrada do catálogo: o nome estável que o layout grava
...
// DashboardWidgets é o catálogo de widgets do painel (spec §5). A permissão de
...
// IsKnownDashboardWidget informa se o nome existe no catálogo.
func IsKnownDashboardWidget(name string) bool {
...
// DashboardWidgetPermission devolve a permissão exigida por um widget (vazia
// quando ele não exige nenhuma) e se o widget existe.
func DashboardWidgetPermission(name string) (string, bool) {
	for _, w := range DashboardWidgets {
		if w.Name == name {
...
// DefaultDashboardLayout é o painel de fábrica: saúde, WANs e alertas na
...
func DefaultDashboardLayout() []LayoutItem {
	return []LayoutItem{
...
// SanitizeDashboardLayout descarta ITEM A ITEM o que não dá para desenhar: nome
...
func SanitizeDashboardLayout(items []LayoutItem) []LayoutItem {
	out := make([]LayoutItem, 0, len(items))
	seen := make(map[string]bool, len(items))
	for _, it := range items {
		if len(out) >= DashboardMaxItems {
			break
		}
		if !IsKnownDashboardWidget(it.Widget) {
			continue
		}
		...
		if it.X+it.W > DashboardGridColumns || it.H > DashboardMaxRowSpan {
			continue
		}
		seen[it.Widget] = true
		out = append(out, it)
...
// operador.
func (db *DB) GetDashboardLayout(userID string) ([]LayoutItem, error) {
	var raw string
	...
	return SanitizeDashboardLayout(items), nil
}
...
func (db *DB) SaveDashboardLayout(userID string, items []LayoutItem) error {
	if userID == "" {
	...
	if len(items) > DashboardMaxItems {
		return fmt.Errorf("layout do painel com %d itens (máximo %d)", len(items), DashboardMaxItems)
	}
