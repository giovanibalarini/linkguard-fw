...
// internal/timesync's persisted config has ServeLAN=true, the DHCP reload
// path must pass the firewall's LAN gateway IP through to the Kea
// generator as the ntp-servers option value.
func TestDoReloadIncludesNTPServerOptionWhenServeLANEnabled(t *testing.T) {
	h, p := newTestNetsvcHandlerWithProvider(t)

	ntpCfg := timesync.Config{ServeLAN: true}
