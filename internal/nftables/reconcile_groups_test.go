...
func inputLines(groups []StoredGroup, ntpNetworks []string, ntpServing bool) []string {
	var lines []string
	for _, toks := range inputChainRules(groups, ntpNetworks, ntpServing, PolicyAccept, AdminAccess{}, nil, false) {
		lines = append(lines, strings.Join(toks, " "))
	}
	return lines
}
...
func TestGroupScopeDecidesWhichChainItLandsIn(t *testing.T) {
	...
	inp := renderChainScript(InputChain, inputChainRules(groups, nil, false, PolicyAccept, AdminAccess{}, nil, false))
