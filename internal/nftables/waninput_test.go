...
func TestSemWANConhecidaNadaEhEmitido(t *testing.T) {
	if r := WANInputRules(Zone{}, gerencia, false, true); r != nil {
		t.Errorf("lista vazia gerou regras: %v", linhas(r))
	}
	if r := WANInputRules(zonaOnPrem("", ""), gerencia, false, true); r != nil {
		t.Errorf("só nomes vazios geraram regras: %v", linhas(r))
	}

	// E a chain inteira continua a de sempre.
	got := linhas(inputChainRules(nil, nil, false, PolicyAccept, AdminAccess{}, Zone{}, false, true))
...
func TestODescarteCasaSomenteConexaoNova(t *testing.T) {
	...
	regras := linhas(WANInputRules(zonaOnPrem("wan0"), gerencia, false, true))
...
func TestWireGuardIsAcceptedOnEveryWANBeforeTheDefaultDrop(t *testing.T) {
	rules := linhas(WANInputRules(zonaOnPrem("wan0", "wan1"), gerencia, false, true, 51820))
...
func TestOAdminNaoPodeSerTrancadoForaPelaLAN(t *testing.T) {
	...
	regras := linhas(WANInputRules(zonaOnPrem("wan0", "wan1"), gerencia, false, true))
...
func TestLiberacoesQueEvitamQuebraDiasDepois(t *testing.T) {
	regras := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), gerencia, false, true)), "\n")
...
func TestODescarteVemDepoisDeTudo(t *testing.T) {
	...
	regras := linhas(inputChainRules([]StoredGroup{g}, nil, false, PolicyAccept, AdminAccess{}, zonaOnPrem("wan0"), false, true))
...
func TestNomeDeInterfaceInseguroEhIgnorado(t *testing.T) {
	...
	regras := linhas(WANInputRules(zonaOnPrem("wan0; drop", "wan1"), gerencia, false, true))
	...
	if r := WANInputRules(zonaOnPrem("wan0; drop"), gerencia, false, true); r != nil {
		t.Errorf("nome inseguro sozinho gerou regra: %v", linhas(r))
	}
}

func TestInterfaceRepetidaNaoViraRegraRepetida(t *testing.T) {
	regras := linhas(WANInputRules(zonaOnPrem("wan0", "wan0"), gerencia, false, true))
...
func TestPortasDeGerenciaNuncaSaoDescartadas(t *testing.T) {
	...
	regras := linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{PanelPort: 9997}, false, true))
	var libera string
	for _, r := range regras {
...
func TestPortaDoPainelNaoEhFixa(t *testing.T) {
	...
	regras := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{SSHPorts: []int{2222}, PanelPort: 8443}, false, true)), "\n")
	...
	uma := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{SSHPorts: []int{9997}, PanelPort: 9997}, false, true)), "\n")
...
func TestAPortaDoSSHNaoEhFixaEmVinteEDois(t *testing.T) {
	...
	regras := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{SSHPorts: []int{2222}, PanelPort: 9997}, false, true)), "\n")
	...
	for _, r := range linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{SSHPorts: []int{2222}, PanelPort: 9997}, false, true)) {
	...
	duas := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{SSHPorts: []int{22, 2222}, PanelPort: 9997}, false, true)), "\n")
	...
	vazio := strings.Join(linhas(WANInputRules(zonaOnPrem("wan0"), AdminAccess{PanelPort: 9997}, false, true)), "\n")
...
func TestFecharAGerenciaTiraSoALiberacaoDaWAN(t *testing.T) {
	...
	aberta := linhas(WANInputRules(zonaOnPrem("wan0"), acesso, false, true))
	fechada := linhas(WANInputRules(zonaOnPrem("wan0"), acesso, true, true))
